...
def get_parser():
    parser = argparse.ArgumentParser(
        ...
        "The following metrics will be computed:\n {}".format(
            FNAME_LOG, list(metric_to_field.keys())
        ),
    ...
    parser.add_argument(
        "-exclude",
        required=False,
        help="R|Config yaml file listing subjects (starting with 'sub') or sites to remove from the statistics."
    ...
    for i in tqdm.tqdm(
        ...
        if not remove_subject(subject, metric, dict_exclude_subj):
            ...
            if site not in results_agg.keys():
                ...
                results_agg[site][
                    "site"
                ] = site  # need to duplicate in order to be able to sort using vendor AND site with Pandas
                results_agg[site]["vendor"] = participants["manufacturer"][
                    rowIndex
                ].array[0]
                results_agg[site]["model"] = participants["manufacturers_model_name"][
                    rowIndex
                ].array[0]
...
def add_stats_per_vendor(
    ax, x_i, x_j, y_max, mean, std, ci, cov_intra, cov_inter, f, color, show_ci=False
):
    """ "
...
def compute_statistics(df):
    ...
    df[
        "mean"
    ] = mean_per_row  # mean within each site (e.g., if there are 35 sites, this will be a vector of length 35)
    df["std"] = std_per_row
    ...
    for vendor in vendors:
        ...
        stats["95ci"][vendor] = (
            1.96 * np.std(val_per_vendor) / np.sqrt(len(val_per_vendor))
        )
        ...
        stats["cov_intra"][vendor] = np.mean(
            df["std"][(df["vendor"] == vendor) & ~df["exclude"]].values
            / df["mean"][(df["vendor"] == vendor) & ~df["exclude"]].values
        )
        # ANOVA: category=[site]
        values_per_site = [
            df["val"][(df["vendor"] == vendor) & (df["site"] == i_site)][0]
            for i_site in df["site"][(df["vendor"] == vendor) & ~df["exclude"]]
        ]
        stats["anova_site"][vendor] = f_oneway(*values_per_site)
        logger.info(
            "ANOVA[site] for {}: {}".format(vendor, stats["anova_site"][vendor])
        )

    # ANOVA: category=[vendor]
    stats["anova_vendor"] = f_oneway(
        *[
            df["mean"][(df["vendor"] == i_vendor) & ~df["exclude"]]
            for i_vendor in vendors
        ]
    )
...
def output_text(stats):
    ...
    if not single_subject:
        txt += (
            "The intra-site COVs were averaged for each vendor and found to be all under "
    ...
    if p_val_anova < 0.05:
        ...
        txt += (
            "The inter-vendor difference was significant (p{}), with the Tukey test showing significant "
        ...
        for counter in range(1, 4):
            ...
            if stats["tukey_test"]._results_table[counter][6].data:
                vendor1 = (
                    stats["tukey_test"]._results_table[counter][0].data
                )  # 1st vendor
                vendor2 = (
                    stats["tukey_test"]._results_table[counter][1].data
                )  # 2nd vendor
                p_adj = (
                    stats["tukey_test"]._results_table[counter][3].data
                )  # adjusted p-val
    ...
    # Inter-vendor difference was not significant
        ...
        p_val_anova = format_p_value(p_val_anova)
        txt += "The inter-vendor difference was not significant (p{})".format(
            p_val_anova
        )
...
def compute_age_statistics():
    ...
    logger.info(
        "..., age between {} and {} y.o., median age {} y.o..".format(
            age_stats["min"], age_stats["max"], age_stats["median"]
        )
    )


def generate_figure_metric(
    df, metric, stats, display_individual_subjects, show_ci=False
):
    """
    ...
    def add_flag(coord, name, ax):
        ...
        ab = AnnotationBbox(
            im, (coord, ax.get_ylim()[0]), frameon=False, pad=0, xycoords="data"
        )
    ...
    bar_plot = plt.bar(
        range(len(site_sorted)),
        height=mean_sorted,
        width=0.5,
        tick_label=site_sorted,
        yerr=[[0 for v in std_sorted], std_sorted],
        color=list_colors,
    )
...
def generate_figure_metric_plotly(df, metric, stats):
    ...
    fig.add_trace(
        go.Bar(
            ...
            textangle=-90,
            error_y=dict(array=std_sorted, color="#000000", width=3),
...
def generate_figure_t1_t2(df, csa_t1, csa_t2):
    ...
    for index, line in enumerate(csa_t1):
        ...
        for subject in csa_t1[index][4]:
            if subject in csa_t2[index, 4]:
                CSA_dict[vendor + "_t1"].append(
                    csa_t1[index, 3][csa_t1[index, 4].index(subject)]
                )
                CSA_dict[vendor + "_t2"].append(
                    csa_t2[index, 3][csa_t2[index, 4].index(subject)]
                )
    ...
    for index, vendor in enumerate(list(OrderedDict.fromkeys(vendor_sorted))):
        ...
        plt.text(
            ...
            0.9,
            "y = {}x + {}\nR\u00b2 = {}".format(
                format_number(slope), format_number(intercept), format_number(r2_sc)
            ),
            ha="left",
            va="center",
...
def generate_figure_t1_t2_plotly(df, csa_t1, csa_t2):
    ...
    fig_v.add_trace(
        go.Scatter(
            x=x, y=y, line=dict(color="black", width=2, dash="dash"), showlegend=False
        )
    )
    fig_v.update_layout(showlegend=True, yaxis_title="T1w CSA", xaxis_title="T2w CSA")
    ...
    for index, vendor in enumerate(list(OrderedDict.fromkeys(vendor_sorted))):
        ...
        fig_2.update_yaxes(
            range=[lim_min - offset, lim_max + offset],
            title_text="T1w CSA",
            row=1,
            col=i,
        )
        ...
        linear_regression.fit(
            (np.array(x).reshape(-1, 1)).reshape(-1, 1),
            (np.array(y).reshape(-1, 1)).reshape(-1, 1),
        )
        intercept = linear_regression.intercept_
        slope = linear_regression.coef_
...
def get_env(file_param):
    ...
    env = {}
    p = subprocess.Popen(
        "env", stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    oldEnv = p.communicate()[0].decode("utf-8")
    p = subprocess.Popen(
        "source {} ; env".format(file_param),
...
def main(argv=sys.argv[1:]):
    ...
    if args.path_results is not None:
        if os.path.isdir(args.path_results):
            ...
            raise FileNotFoundError(
                "Directory '{}' was not found.".format(args.path_results)
            )
    ...
    for csv_file in file_to_metric.keys():
        ...
        # Generate figure
        generate_figure_metric(
            df, metric, stats, display_individual_subjects, show_ci=args.show_ci
        )
