...
def correct_segmentation(fname, fname_seg_out):
    ...
    # launch FSLeyes
    print(
        "In FSLeyes, click on 'Edit mode', correct the segmentation, then save it with the same name (overwrite)."
    )
...
def correct_vertebral_labeling(fname, fname_label):
    ...
    os.system(
        "sct_label_utils -i {} -create-viewer 3,5 -o {} -msg {}".format(
            fname, fname_label, message
        )
    )
...
def main():
    ...
    if not args.qc_only:
        name_rater = input(
            "Enter your name (Firstname Lastname). It will be used to generate a json sidecar with each "
            "corrected file: "
        )
        ...
        if files is not None:
            for file in files:
                ...
                fname_label = os.path.join(
                    ...
                    subject,
                    contrast,
                    sg.utils.add_suffix(file, get_suffix(task, "-manual")),
                )
                os.makedirs(
                    os.path.join(path_out_deriv, subject, contrast), exist_ok=True
                )
                if not args.qc_only:
                    ...
                    if do_labeling:
                        if task in ["FILES_SEG", "FILES_GMSEG"]:
                            ...
                            sys.exit(
                                "Task not recognized from yml file: {}".format(task)
                            )
                ...
                # generate QC report
                os.system(
                    "sct_qc -i {} -s {} -p {} -qc {} -qc-subject {}".format(
