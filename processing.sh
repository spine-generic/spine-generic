...
echo "Creating t1 labels"
sct_label_utils -i t1.nii.gz -create-viewer 3,8

# Create mask for faster processing
sct_create_mask -i t1.nii.gz -p centerline,t1_seg.nii.gz -size 40mm -o mask_cord.nii.gz

# Crop data for faster processing
echo "Cropping t1 data"
sct_crop_image -i t1.nii.gz -m mask_cord.nii.gz -o t1_crop.nii.gz
sct_crop_image -i t1_seg.nii.gz -m mask_cord.nii.gz -o t1_seg_crop.nii.gz
...
echo "Creating vertebral levels for t1"
sct_label_vertebrae -i t1_crop.nii.gz -s t1_seg_crop -c t2 -initc2

# Register to template
echo "Registering t1 to template"
sct_register_to_template -i t1_crop.nii.gz -s t1_seg_crop.nii.gz -c t1 -l labels_crop.nii.gz

# Warp template without white matter atlas...
echo "Warping template to t1"
